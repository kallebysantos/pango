...
public sealed class ComponentRegisterCreation : AsyncCommand<ComponentRegisterCreation.Settings>
{
  ...
  )
  {
    ...
    foreach (var source in settings.ComponentsSource)
    {
      ...
      registryIndex.Add(component.Name);
    }

    var indexFilePath = Path.Combine(outputDir.FullName, "index.json");
    await using var indexFile = File.Create(indexFilePath);
    await JsonSerializer.SerializeAsync(
        utf8Json: indexFile,
        value: registryIndex,
        options: new(JsonSerializerDefaults.Web)
