...
[gold1]                          |___/          [/][orangered1]   /    (_(_\________//   [/]"
    ...
    AnsiConsole.MarkupInterpolated(
        $"[bold]Capture ants... I mean, components to your project[/]\n\n"
    );
  }

  config
      .AddCommand<DownloadComponent>("add")
      .WithDescription("Downloads a component from the specified registry.")
      .WithExample("add", "button");

  config.AddCommand<ListComponents>("list")
      .WithDescription("List components from the specified registry");

  config
      .AddCommand<ConfigurationInit>("init")
      .WithDescription("Creates a Pango configuration file.")
      .WithExample(
          "init",
          "-n MyProject.Components",
          "-o src/MyProject.Components",
          "--registry-uri https://raw.github.com/repo"
      );
