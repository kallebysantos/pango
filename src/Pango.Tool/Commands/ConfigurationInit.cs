...
public sealed class ConfigurationInitSettings : CommandSettings
{
  ...
  [NotNull]
  [Description("Components to download.")]
  [CommandOption("-c|--components")]
...
public sealed class ConfigurationInit : AsyncCommand<ConfigurationInitSettings>
{
  ...
  public static async Task<Result<IOk, IError>> Init(ConfigurationInitSettings settings)
  {
    ...
    var savedConfigResult = await savedConfig
        ...
        .AndThen(async _ =>
          isPangoRegistry && AnsiConsole.Confirm("Would like to add [bold]PageScript[/] component?")
          ? await DownloadComponent.AddComponent(config, "page-script")
          : new OkResult()
      );

    var result = await savedConfigResult
      .AndThen(async _ =>
        AnsiConsole.Confirm("Would like to add more components?")
        ? await ShowComponentList(config)
        : new OkResult()
      );
  ...
  static async Task<Result<IOk, IError>> ShowComponentList(LocalConfig config)
  {
    return await ListComponents.List(
        new()
        {
